...
namespace ABSoftware.ABSave.Deserialization
{
    ...
    public class ABSaveDeserializer
    {
        ...
        /// <param name="errHandler">The way of handling errors.</param>
        /// <param name="location">OPTIONAL: This is used to add a location to the error from the <paramref name="errHandler"/></param>
        /// <returns>The deserialized object - can be null if it needs to be manually parsed.</returns>
        public static object Deserialize(string str, Type objType, ABSaveErrorHandler errHandler = null, int location = 0)
        {
            return Deserialize(str, objType, out ABSavePrimitiveType none, out bool none2, errHandler, location);
        }
        ...
        /// <param name="location">OPTIONAL: This is used to add a location to the error from the <paramref name="errHandler"/></param>
        /// <returns>The deserialized object - can be null if it needs to be manually parsed.</returns>
        public static object Deserialize(string str, Type objType, out ABSavePrimitiveType determinedType, out bool manuallyParse, ABSaveErrorHandler errHandler = null, int location = 0)
        {
            errHandler = ABSaveErrorHandler.EnsureNotNull(errHandler);
            ...
            else if (ABSaveUtils.IsNumericType(objType))
            {
                determinedType = ABSavePrimitiveType.Number;
                return DeserializeNumber(str, objType, errHandler, location);
            }
            ...
            else if (objType == typeof(bool))
            {
                ...
                return DeserializeBool(str, errHandler, location);
            }
            ...
            else if (objType == typeof(DateTime))
            {
                ...
                return DeserializeDateTime(str, errHandler, location);
            }
        ...
        /// </summary>
        /// <param name="str">The string to deserialize.</param>
        /// <param name="errHandler">The error handler used to handle errors.</param>
        ...
        public static T DeserializeNumber<T>(string str, ABSaveErrorHandler errHandler = null, int location = 0)
        {
            return (T)DeserializeNumber(str, typeof(T), errHandler, location);
        }
        ...
        /// <returns>The deserialized value.</returns>
        public static object DeserializeNumber(string str, Type targetType, ABSaveErrorHandler errHandler = null, int location = 0)
        {
            errHandler = ABSaveErrorHandler.EnsureNotNull(errHandler);

            // Attempt to parse it.
            var passed = double.TryParse(str, out double result);

            // If it fails to parse, the value isn't valid, so that will throw an error.
            if (!passed)
            {
                errHandler.InvalidValueInABSaveWhenParsing(location, "The number given: " + str + " is not valid for the type: " + targetType.Name);
                return null;
            }
        ...
        public static bool DeserializeBool(string str, ABSaveErrorHandler errHandler = null, int location = 0)
        {
            errHandler = ABSaveErrorHandler.EnsureNotNull(errHandler);
            ...
            // Otherwise, there's a problem.
                ...
                errHandler.InvalidValueInABSaveWhenParsing(location, "The boolean given: " + str + " is not valid.");
        ...
        public static DateTime DeserializeDateTime(string str, ABSaveErrorHandler errHandler = null, int location = 0)
        {
            errHandler = ABSaveErrorHandler.EnsureNotNull(errHandler);

            // Just return a new DateTime, but with the number deserialized.
            return new DateTime(DeserializeNumber<long>(str, errHandler, location));
        }
    }
}
